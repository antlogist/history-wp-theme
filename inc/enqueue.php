...
function history_styles_and_scripts() {
  ...
  //custom CSS
  wp_enqueue_style('history-app-css', get_theme_file_uri('dist/css/all.min.css'), null, null);
  ...
  if('newsletter' === get_post_type() || 'will' === get_post_type()) {
  ...
  //custom js
  wp_enqueue_script('history-app-js', get_theme_file_uri('dist/js/all.min.js'), array( 'jquery' ), null, true);

  //masonry js
  wp_enqueue_script('masonry-js', get_theme_file_uri('dist/js/masonry.pkgd.min.js'), null , '' , true);

  //vue
  if (is_page('shop') || is_page('cart') || is_page('product') || is_page('checkout') || is_page('view-order')) {
